...
function generateQuestionnairePDF(q: Questionnaire): void {
  ...
  const partialCount = allQuestions.filter(
    (q) => q.answer === "partial",
  ).length;
  ...
  for (const section of q.sections) {
    ...
    const sectionYes = section.questions.filter(
      (q) => q.answer === "yes",
    ).length;
    doc.setFontSize(9);
    doc.setTextColor(...MUTED_TEXT);
    doc.text(
      `${sectionYes}/${section.questions.length} controls satisfied`,
      14,
      27,
    );
...
function generateComplianceReport(): void {
  ...
  for (const domain of config.controls || []) {
    ...
    const implemented = domain.items.filter(
      (i: any) => i.status === "implemented",
    ).length;
    doc.setFontSize(9);
    doc.setTextColor(...MUTED_TEXT);
    doc.text(
      `${implemented}/${domain.items.length} controls implemented`,
      14,
      27,
    );
  ...
  autoTable(doc, {
    ...
    body: (config.subprocessors || []).map((s: any) => [
      s.name,
      s.purpose,
      s.location,
    ]),
